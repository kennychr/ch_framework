...

{
	...
	switch (_type) do
	{
		case "ARRAY": { [_x, _behaviours] call AW_fnc_ai_setBehaviour; };
		...
		default
		{
			...
			{
				_pos = [_pos, ((markerSize _pos) call BIS_fnc_lowestNum)] call AW_fnc_position_randomPosTrigger;
			};
			...
			switch (_order) do
			{
				...
				case "patrol":
				{
					...
					while { !_didWork && _attempts < _attempt_limit } do
					{
						...
						_moveWP = _group addWaypoint [_pos, 30]; //Tryout: Battle static syndrome|						
