...
// Grab enemy units
_spawnedUnits = [];
_soldiers = getArray (missionConfigFile >> "AW_Enemy" >> "infantry");
_vehicles = getArray (missionConfigFile >> "AW_Enemy" >> "vehicles");
_static = getArray (missionConfigFile >> "AW_Enemy" >> "static");
_boats = getArray (missionConfigFile >> "AW_Enemy" >> "boats");
_air = getArray (missionConfigFile >> "AW_Enemy" >> "air");
...
for [{_i = 0}, {_i < _amount}, {_i = _i + 1}] do
{
	...
	_pos = switch (_lName) do
	{
		...
		case "STRING":
		{
			_tempPos = [_loc] call AW_fnc_position_randomPosTrigger;
	...
	switch (_type) do
	{
		case "infantry":
		{
			...
			{
				_x addEventHandler ["killed", { (_this select 0) call AW_fnc_garbage_addDead; }];
