/*
	...
	Parameters
		...
		|				-	"shore"
		|				-	"land"
		|				- "land2" uses mapCenter and definable radius
		|				-	"civilisation"
		|				-	"anywhere"
		|				-	"main"
...
while {!_validPos} do
{
	...
	switch (_area) do
	{
		...
		case "land2": { _pos = [AW_MAP_CENTER, 0, 5000, _radius, 0, _gradient, 0] call BIS_fnc_findSafePos; };
	...
	{
		if ((_x distance _pos) <= 1500) exitWith { _validPos = false; };
